...
function [Gopt,error_flag]=CLE_optimal(G,ind_root)

ncount=max(str2num(char(G.Nodes.Name))); % the max node number so can add new nodes with different names
...
while ~flag_opt
    iter=iter+1;
    [G1,G2,KickOut,flag_opt]=CLE_calc1(G2,KickOut,ncount);
...
%     title('G2')
    ...
    try % should work if ind_root is strongly connected to other nodes -otherwise will need to modify that choic

%% determine sample-time matches for individual VIC GISAID seqs 
% using gender and age data when available

function idN=prob_link_VIC_noany(T_Gst,T_Nst,p_time_diff)
...
if ~isempty(psex)
   ...
   p_age_VIC=zeros(height(ages),1); 
    ...
    if ~ismissing(age)
        ...
        % assume missing age means the person is <18 
        iiN1=ages<18;
        if any(iiN1)
            p_age_VIC(iiN1)=minpdiff; % keep this rather than uniform dist so biasing toward time fitting
        end
    end
   % calculate their probs dependent on the matching mechanism
    p_time=p_time_diff(ddays-mindiff+1,2);
 
    prob=p_time.*psex.*p_age_VIC;
